...
//	On construit le triangle à partir des points
...
//	Constructeur par copie du triangle à partir d'une forme Triangle
	
	public Triangle(String nom, Forme f) throws ClassCastException {
		
//		Le try-catch sert à vérifier si la forme à copier est bien un triangle
		
		try {
			Triangle t = ((Triangle)f);
			if(nom != f.getNomForme()) {
				super.setNomForme(nom);
				this.p1 = new PointPlan(t.getPoint(0).getAbscisse(), t.getPoint(0).getOrdonnee());
		        this.p2 = new PointPlan(t.getPoint(1).getAbscisse(), t.getPoint(1).getOrdonnee());
		        this.p3 = new PointPlan(t.getPoint(2).getAbscisse(), t.getPoint(2).getOrdonnee());
			}
...
//		Exceptions:
//			IndexOutOfBoundsException si l'indice i est en dehors de la liste des PointPlans de la classe Triangle
...
//		Retourne la liste des points du triangle (Cette méthode est privée car l'utilisateur de la classe Chapeau n'aura pas besoin de cette méthode qui est juste une question de simplicité pour les développeurs de la classe)
...
//		Retourne une liste des segments qui dessinent le triangle
...
//		Déplace le triangle aux coordonnées données x et y
...
//		Initialiser un triangle avec le clavier
...
//		Retourne une chaine de caracteres qui représente le triangle
